...
public class CowActivity extends Activity implements MediaPlayer.OnCompletionListener{
	...
	private void play() {
		stop();
		MPlayer.start();
		tombol_suara.setImageResource(R.drawable.cow_spell_2);
		tombol_suara.setEnabled(false);
	}
	private void stop() {
		MPlayer.stop();
		tombol_suara.setImageResource(R.drawable.cow_spell_1);
		try {
			MPlayer.prepare();
			MPlayer.seekTo(0);
