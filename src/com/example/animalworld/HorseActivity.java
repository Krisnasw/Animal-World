...
public class HorseActivity extends Activity implements MediaPlayer.OnCompletionListener{
	...
	private void play() {
		stop();
		MPlayer.start();
		tombol_suara.setImageResource(R.drawable.horse_spell_2);
		tombol_suara.setEnabled(false);
	}
	private void stop() {
		MPlayer.stop();
		tombol_suara.setImageResource(R.drawable.horse_spell_1);
		try {
			MPlayer.prepare();
			MPlayer.seekTo(0);
	...
	private void buka_file_suara() {
		try {
			MPlayer = MediaPlayer.create(this, R.raw.horse);
