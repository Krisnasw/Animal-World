...
public class ChimpanzeeActivity extends Activity implements MediaPlayer.OnCompletionListener{
	private ImageButton tombol_suara;
	private MediaPlayer MPlayer;
	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_chimpanzee);
	...
	private void play() {
		stop();
		MPlayer.start();
		tombol_suara.setImageResource(R.drawable.chimpanzee_spell_2);
		tombol_suara.setEnabled(false);
	}
	private void stop() {
		MPlayer.stop();
		tombol_suara.setImageResource(R.drawable.chimpanzee_spell_1);
		try {
			MPlayer.prepare();
			MPlayer.seekTo(0);
	...
	private void buka_file_suara() {
		try {
			MPlayer = MediaPlayer.create(this, R.raw.chimpanzee);
